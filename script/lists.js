...
async function fetchLists() {
    ...
    try {
        const response = await fetch(`http://localhost:8080/api/shopping?username=${usuario}`, { 
    method: 'GET',
    headers: {
...
async function deleteList(listId) {
    ...
    try {
        const response = await fetch(`http://localhost:8080/api/shopping/${listId}`, {  // URL da API para deletar listas
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
